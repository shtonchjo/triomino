...
function calculateTriominoSize() {
  ...
  if ((width - 2 * margin) < (edgeTs[1] - edgeTs[0])) {
    ...
    unzoom(min(1, factorX, factorY), boardCenter()[0], boardCenter()[1])
  }
  let factorY = 1
...
function edgeTriominos() { // If no placedTriomino, return false
  if (placedTriominos.length == 0) {
    return false
  }
...
function moveAllPlacedTriominos(dx = 0, dy = 0) {
  for (let i = 0; i < placedTriominos.length; i++) {
...
function mouseClicked() {
  message = "";
  if (activeTriomino) { // sequence to position the active triomino
  ...
  } else { // check clicking on player triominos or on a button
...
class Triomino {
  ...
  // belongsToTriangles(pos) {
  //   if (triangles.length == 0){
  //     return true
  //   }
  //   for (let t = 0; t < triangles.length; t++) {
  //     if (pos[0] == triangles[t][0] && pos[1] == triangles[t][1]) {
  //       return true
  //     }
  //   }
  //   return false
  ...
  showNeighborLocations() {
    ...
    for (let i = -1; i <= 1; i++) {
      if (i == 0){
        rot = (placedTriominos[i].rot % 2 == 0 ? -1 : 1)
        pos = [this.pos[0]+i,this.pos[1]+rot]
      } else {
        pos = [this.pos[0]+i,this.pos[1]]
      }
      if (!this.alreadyTriominoHere(pos)) {
      loc = this.neighbor(i)
  ...
  clickOnNeighbor(loc = -1) {
    let neighborTriangle = this.neighbor(loc);
    if (neighborTriangle) {
      return this.inTriangle(mouseX, mouseY, neighborTriangle)
    }
    return false
  }
  inTriangle(x, y, l) { // is [x,y] in the triangle [x1,y1,x2,y2,x3,y3]
...
class Player {
  constructor(name, index,AI = false) {
...
//   function belongsToTriangles(pos) {
//     for (let t = 0; t < triangles.length; t++) {
//       if (pos[0] == triangles[t][0] && pos[1] == triangles[t][1]) {
//         return true
//       }
...
//   }
//   for (let i = 0; i < placedTriominos.length; i++) {
//     let onTopOrBottom = (placedTriominos[i].rot % 2 == 0 ? 1 : -1)
//     let leftTriangle = [placedTriominos[i].pos[0] - 1, placedTriominos[i].pos[1]]
//     if (!belongsToTriangles(leftTriangle) && !alreadyTriominoHere(leftTriangle)) {
//       triangles.push(leftTriangle)
//     }
//     let rightTriangle = [placedTriominos[i].pos[0] + 1, placedTriominos[i].pos[1]]
//     if (!belongsToTriangles(rightTriangle) && !alreadyTriominoHere(rightTriangle)) {
//       triangles.push(rightTriangle)
//     }
//     let topOrBottomTriangle = [placedTriominos[i].pos[0], placedTriominos[i].pos[1] + onTopOrBottom]
//     if (!belongsToTriangles(topOrBottomTriangle) && !alreadyTriominoHere(topOrBottomTriangle)) {
//       triangles.push(topOrBottomTriangle)
//     }
//   }
// }
...
//   function getXYofOrigin() {
//     for (let t = 0; t <= placedTriominos.length; t++) {
//       if (placedTriominos[t].pos[0] == 0 && placedTriominos[t].pos[1] == 0) {
//         return [placedTriominos[t].coord.x, placedTriominos[t].coord.y]
//       }
//     }
//     return false
//   }

//   let origin = getXYofOrigin()

//   function vertices(x , y , rot , size ) {
//     let vert = [];
//     for (let theta = HALF_PI + rot * PI; theta < HALF_PI + TWO_PI + rot * PI; theta += TWO_PI / 3) {
//       vert.push(x - 7 / 6 * size * cos(theta));
//       vert.push(y - 7 / 6 * size * sin(theta));
...
//     let rot = (pos[0] + pos[1] + 1) % 2
//     let x = origin[0] + (4 / 3 * Tsize_board) * cos(pos[1] * TWO_PI / 3 - PI / 2);
//     let y = origin[1] + pow(-1, rot) * (4 / 3 * Tsize_board) * sin(pos[1] * TWO_PI / 3 + PI / 2);
//     console.log(x,y)
//     return vertices(x, y, rot + 1, Tsize_board * .9);
//   }
...
//   for (let t = 0; t <= triangles.length; t++) {
//     if (triangles[t]) {
//       loc = trianglePoints(triangles[t])
//       triangle(loc[0], loc[1], loc[2], loc[3], loc[4], loc[5])
//     }
//   }
// }