...
function setup() {
  ...
  cBtnLebensraum = new CanvasButton("\u2b81","")
...
function draw() {
  ...
  //   // let minX = height * buttonHeightRatio;
  //   // let maxX = width - 2.2*height * buttonHeightRatio - 1;
  //   cBtnLebensraum.x = mouseX-cBtnLebensraum.w/2 // min(max(mouseX,minX),maxX);
  //   let minRatio = .1;
  //   playerFieldHeightRatio = max(1-mouseY/height,minRatio);
  //   cBtnLebensraum.y = -cBtnLebensraum.h/2+mouseY;
  //  cBtnLebensraum.show();
  }else {
    noLoop()
  }
...
function calculateTriominoSize() {
  ...
  cBtnLebensraum.param(width - 2.2*buttonDimensions - 1, buttonY+buttonDimensions*.75,buttonDimensions*.35,0,buttonDimensions/2);
  redraw();
}
...
function scrollTyping(force = false) {
  typingScrollY = 100
  if ((typed.length <=1) || force){
  noStroke()
  fill(255, 100)
...
function mouseClicked() {
  ...
  } else if (cBtnLebensraum.click()){
    //oop()
    if (activeLebensraum){
      cBtnLebensraum.clicked = false
      activeLebensraum = false
    } else {
      activeLebensraum = true
      //loop()
      //draw()
    }
      cBtnLebensraum.show();
...
class CanvasButton {
  ...
  show() {
    ...
    text(this.text, this.x + this.w / 2, this.y + this.h *.55);
  }
  click() {
...
//TODO bar to extend the player territory
